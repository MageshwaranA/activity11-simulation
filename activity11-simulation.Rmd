...
```{r}
norm_samp_1000 %>% 
  summarise(n1000_mean = mean(norm_samp_1000$values),n1000_sd = sd(norm_samp_1000$values))
```
...
```{r}
norm_samp_30 <- tibble(
  valuess = rnorm(n = 30, mean = 50, sd = 10))
  
norm_samp_30 %>% 
  summarise(n30_mean = mean(norm_samp_30$valuess),n30_sd = sd(norm_samp_30$valuess))
...
non_norm_samp_1000 <- tibble(
  valuesss = rexp(n = 1000, rate = 0.02)
)
non_norm_samp_1000 %>% 
  summarise(non_n1000_mean = mean(non_norm_samp_1000$valuesss),non_n1000_sd = sd(non_norm_samp_1000$valuesss))

non_norm_samp_30 <- tibble(
  valuessss = rexp(n = 30, rate = 0.02)
)
non_norm_samp_30 %>% 
  summarise(non_n30_mean = mean(non_norm_samp_30$valuessss),non_n30_sd = sd(non_norm_samp_30$valuessss))
...
```{r}
generate_n_samples <- function(size,mean,std_dev,prate,pdf,pmin,pmax){
  final <- tibble::tibble(
    Ndistr = rnorm(n = size, mean = mean, sd = std_dev),
    Edistr = rexp(n = size, rate = prate),
    Csqure = rchisq(n = size, df = pdf),
    Udistr = runif(n = size, min = pmin, max = pmax)
  )
  return (final)
}
```
